...
class ClassifiedSwipeDetail extends StatefulWidget {
  const ClassifiedSwipeDetail({required Key key}) : super(key: key);

  @override
  State<ClassifiedSwipeDetail> createState() => _ClassidiedDetailsScreenState();
}

class _ClassidiedDetailsScreenState extends State<ClassifiedSwipeDetail> {
  ...
  void fetchPostDetails(int postId) {
    // Make sure the postId is valid and not zero
  ...
  void initializeLikes() {
    isLiked =
        RxBool(controller.classifiedList[currentPage].likedByUser ?? false);
    likeCount = RxInt(controller.classifiedList[currentPage].totallike ?? 0);
  }

  void initializeBookmarks() {
    isBookmarked = RxBool(
        controller.classifiedList[currentPage].bookmarkedByUser ?? false);
    bookmarkCount =
        RxInt(controller.classifiedList[currentPage].totalbookmark ?? 0);
  }
  ...
  var currentPost = Rxn<GetClassifiedData>();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: buildBottomNavigationBar(size),
    );
  }

  Widget buildPostContent(Size size) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Padding(
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      postTimeFormatter.formatPostTime(
                        DateTime.parse(post.createdate ?? '').isAtSameMomentAs(
                                DateTime.parse(post.datemodified ?? ''))
                            ? post.createdate ?? ''
                            : post.datemodified ?? '',
                      ),
                      style: textStyleW400(
