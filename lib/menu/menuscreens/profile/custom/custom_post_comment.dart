...
class CommentDialogPost extends StatefulWidget {
  ...
  const CommentDialogPost({super.key, required this.postId});
...
class _CommentDialogState extends State<CommentDialogPost> {
  ...
  Future<void> _refreshData() async {
    controller.getCommentPost(1, widget.postId, context);
  }
  ...
  void _scrollListener() {
    if (_scrollController.position.pixels ==
      ...
      controller.getCommentPost(nextPage, widget.postId, context);
    }
  }
                              ...
                              Padding(
                                ...
                                child: GestureDetector(
                                  onTap: () async {
                                    ...
                                    await controller.addReplyPostComment(
                                        widget.postId, 0, context);
  ...
  Widget _buildComment(GetUserPostCommentData comment, Size size) {
    currentUserID = currentUserID ?? 0;

    bool isCurrentNewsUserComment = comment.userid == currentUserID;
    if (kDebugMode) {
      print('Current Post User ID: $currentUserID');
    }
    return Padding(
                          ...
                          TextButton(
                            onPressed: () =>
                              ...
                              await controller.deleteComment(
                                  widget.postId, comment.id ?? 0, context);
                              await _refreshData();
                            }),
                            child: Text(
                              'Delete',
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          if (isReply) {
                            await controller.editComment(widget.postId,
                                data.id ?? 0, editedComment, 'post', context);
                          } else {
                            await controller.editComment(widget.postId,
                                data.id ?? 0, editedComment, 'post', context);
                          }
  ...
  // Example usage:
  void _showEditCommentDialog(GetUserPostCommentData comment) {
    _showEditDialog(comment, false);
  }

  void _showEditReplyDialog(GetUserPostCommentDataCommentsReplays reply) {
    _showEditDialog(reply, true);
  }

  void _showEditReplytoReplyDialog(
      GetUserPostCommentDataCommentsReplaysCommentsReplays reply) {
    _showEditDialog(reply, true);
  }

  Widget _buildSingleReply(
    GetUserPostCommentDataCommentsReplays reply,
    Size size,
  ) {
                          ...
                          if (isCurrentUserReply)
                            TextButton(
                              onPressed: () =>
                                  LogoutDialog.show(context, () async {
                                await controller.deleteComment(
                                    widget.postId, reply.id ?? 0, context);
                                await _refreshData();
                              }),
                              child: Text(
                                'Delete',
...
void showFullScreenDialogPost(BuildContext context, int postId) {
  showGeneralDialog(
    context: context,
    ...
    pageBuilder: (BuildContext buildContext, Animation animation,
      ...
      return CommentDialogPost(postId: postId);
    },
  );
}
